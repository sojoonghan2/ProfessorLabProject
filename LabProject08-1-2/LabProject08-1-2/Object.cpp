...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0)
{
	...
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/WaveFoam.dds", RESOURCE_TEXTURE2D, 1);
#ifdef _WITH_HEIGHT_MAP_TEXTURE
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/HeightMap(Flipped).dds", RESOURCE_TEXTURE2D, 2);
#else
	pTerrainTexture->LoadBuffer(pd3dDevice, pd3dCommandList, m_pHeightMapImage->GetHeightMapPixels(), nWidth * nLength, sizeof(BYTE), DXGI_FORMAT_R8_UNORM, 2);
#endif
...
//	pWaterTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Water_Texture_Alpha.dds", RESOURCE_TEXTURE2D, 2);
	CScene::CreateShaderResourceViews(pd3dDevice, pWaterTexture, 0, 5);
