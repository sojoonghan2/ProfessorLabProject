...
{
	m_fxPitch = 12.0f * 3.5f;
	m_fyPitch = 12.0f * 3.5f;
	m_fzPitch = 12.0f * 3.5f;

	m_xObjects = int(257 * 8.0f / m_fxPitch);
	m_yObjects = 2;
	m_zObjects = int(257 * 8.0f / m_fzPitch);

	m_nObjects = (m_xObjects * m_yObjects * m_zObjects);
}
...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	for (int i = 0, x = 0; x < m_xObjects; x++)
	{
		for (int z = 0; z < m_zObjects; z++)
		{
			for (int y = 0; y < m_yObjects; y++)
			{
...
#endif
				float xPosition = x * m_fxPitch;
				float zPosition = z * m_fzPitch;
				float fHeight = pTerrain->GetHeight(xPosition, zPosition);
				pRotatingObject->SetPosition(xPosition, fHeight + (y * 3.0f * m_fyPitch) + 6.0f, zPosition);
				if (y == 0)
				{
