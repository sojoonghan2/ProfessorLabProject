...
Texture2D<float4> gtxtTerrainIceCuvatureTexture : register(t6);
Texture2D<float> gtxtTerrainAlphaTexture : register(t7);
...
float4 PSTerrain(VS_TERRAIN_OUTPUT input) : SV_TARGET
{
	...
	float4 cIceCurvatureTexColor = gtxtTerrainIceCuvatureTexture.Sample(gWrapSamplerState, input.uv1 * 0.25f);
...
//	float4 cColor = cBaseTexColor * 0.5f + cDetailTexColor * 0.5f;
	float4 cColor = saturate(cBaseTexColor + lerp(cIceCurvatureTexColor, cDetailTexColor, fAlpha));
